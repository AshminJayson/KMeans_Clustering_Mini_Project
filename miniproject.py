...
# number of genders
plt.subplot(1, 2, 1)
figure_4 = sns.barplot(x=['Female', 'Male'],
...
list_genders_spending_score_mean = [int(spending_score_male/df['Gender'].value_counts()[
                                        1]), int(spending_score_female/df['Gender'].value_counts()[0])]
series_genders_spending_score_mean = pd.Series(
    data=list_genders_spending_score_mean)

plt.subplot(1, 2, 2)
figre_5 = sns.barplot(x=['Male (1)', 'Female (0)'],
...
# median
stats.levene(df['Annual Income (k$)'],
             df['Spending Score (1-100)'], df['Age'], center='median')

# mean
stats.levene(df['Annual Income (k$)'],
             df['Spending Score (1-100)'], df['Age'], center='mean')
...
plt.scatter(X_2[labels == 0, 0], X_2[labels == 0, 1],
            s=80, c='green', label='Cluster-1')
plt.scatter(X_2[labels == 1, 0], X_2[labels == 1, 1],
            s=80, c='orange', label='Cluster-2')
plt.scatter(X_2[labels == 2, 0], X_2[labels == 2, 1],
            s=80, c='red', label='Cluster-3')
plt.scatter(X_2[labels == 3, 0], X_2[labels == 3, 1],
            s=80, c='purple', label='Cluster-4')

plt.scatter(centroids[:, 0], centroids[:, 1], s=400,
